            ...
            <div className="py-10">
                ...
                <div className="flex flex-col">
                    <div className="flex flex-col md:flex-row justify-around py-10 items-center">
                        ...
                        className="overflow-hidden rounded-lg items-center pb-5 md:pb-0">
                    ...
                    <div className="flex flex-col md:flex-row-reverse justify-around bg-red-700 py-10 items-center shadow-[0_6px_15px_rgba(0,0,0,0.6)]">
                        ...
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5 }}
                        className="overflow-hidden rounded-lg pb-5 md:pb-0">
                            <img src={picture} alt="" className="w-48 md:w-64 h-auto border border-black hover:scale-110 transition-transform duration-700 ease-out" />
                        </motion.div>
                        <motion.p 
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5 }}
                        className="w-96 text-center text-2xl font-pixel">As the idea started taking shape, we spent countless hours learning, testing, and rebuilding things that broke more times than we can count. There were moments where progress felt painfully slow, but every small improvement pushed us forward. Those struggles taught us patience, teamwork, and how powerful simple persistence can be when everyone is moving toward the same goal.</motion.p>
                    </div>
                    <div className="flex flex-col md:flex-row justify-around py-10 items-center">
                        <motion.div 
                        initial={{ opacity: 0, y: 20 }}
                        ...
                        whileInView={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.5 }}
                        className="w-96 text-red-700 text-center text-2xl font-pixel">What began as a small experiment slowly grew into something we genuinely believe in. The journey is still far from finished, but every step we’ve taken so far reminds us why we started in the first place. This project is more than just an idea now. It’s the result of shared effort, countless lessons, and the determination to keep building something better every single day.</motion.p>
                    </div>
                </div>
            </div>
    )
